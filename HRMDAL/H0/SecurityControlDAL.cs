...
namespace HRMDAL.H0
{
    public class SecurityControlDAL : Dao
    {
        ...
        public DataTable GetAllByFilter(string fullName, string departmentIds, string status, DateTime almostExpired, int isLost)
        {
            Debug.Assert(sproc == null);
            var datatable = new DataTable();
            try
            {
                SqlParameter[] param =
                {
                    ...
                    new SqlParameter("@DepartmentId", SqlDbType.NVarChar, 4000),
                    new SqlParameter("@Status", SqlDbType.NVarChar, 4000),
                    new SqlParameter("@AlmostExpired", SqlDbType.DateTime),
                    new SqlParameter("@IsLost", SqlDbType.Bit),
                };

                param[0].Value = fullName;
                param[1].Value = departmentIds;
                param[2].Value = status;
                param[3].Value = almostExpired;
                param[4].Value = isLost;

                sproc = new StoreProcedure("Sel_H0_SecurityControl_ByFilter", param);
                sproc.RunFill(datatable);
            }
            catch (SqlException se)
            {
        ...
        public long Insert_SCIManagement(int LostId, int SecurityControlId, int UserId, DateTime LostDate, int LostTime, string Remark)
        {
            long identity = 0;
            Debug.Assert(sproc == null);
            try
            {
                SqlParameter[] param =
                {
                    new SqlParameter("@LostId", SqlDbType.Int),
                    new SqlParameter("@SecurityControlId", SqlDbType.Int),
                    new SqlParameter("@UserId", SqlDbType.Int),
                    new SqlParameter("@LostDate", SqlDbType.DateTime),
                    new SqlParameter("@LostTime", SqlDbType.Int),
                    new SqlParameter("@Remark", SqlDbType.NVarChar, 4000)
                };

                param[0].Value = LostId;
                param[1].Value = SecurityControlId;
                param[2].Value = UserId;
                param[3].Value = LostDate;
                param[4].Value = LostTime;
                param[5].Value = Remark;

                sproc = new StoreProcedure("Ins_H0_EmployeeSecurityIdManagement", param);
                identity = sproc.Run();
                sproc.Commit();
            }
            catch (SqlException se)
